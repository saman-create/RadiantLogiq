...
describe('RadiantLogiq application shell', () => {
  it('renders one clear page heading and the primary demo action', () => {
    ...
    expect(screen.getByRole('heading', { level: 1 })).toHaveTextContent('Clinical reasoning for imaging decisions.');
    expect(screen.getAllByRole('link', { name: 'Request Demo' }).some((link) => link.getAttribute('href') === '/demo')).toBe(true);
  });
  ...
  it('keeps every desktop navigation destination reachable', () => {
    ...
    for (const [label, href] of [
      ['Products', '#products'], ['Pricing', '/pricing'], ['About', '/about'],
      ['Partners', '/partners'], ['Blog', '/blog'], ['Contact', '/contact'],
      ['Log in', '/login'], ['Request Demo', '/demo'],
    ]) {
  ...
  it('opens and closes the mobile navigation accessibly', async () => {
    ...
    expect(screen.getByRole('dialog', { name: 'Mobile navigation' })).toHaveAttribute('aria-modal', 'true');
  ...
  it('preserves all legal destinations in the footer', () => {
    ...
    expect(within(footer).getByRole('link', { name: 'HIPAA Compliance' })).toHaveAttribute('href', '/hipaa');
    expect(within(footer).getByRole('link', { name: 'Security' })).toHaveAttribute('href', '/security');
  });

  it('uses the supplied RadiantLogiq logo in the shared wordmarks', () => {
    render(<App />);

    const homeLinks = screen.getAllByRole('link', { name: 'RadiantLogiq home' });
  ...
  it('condenses the navbar into a glass surface after scrolling within the hero', async () => {
    ...
    Object.defineProperty(hero, 'offsetHeight', { configurable: true, value: 900 });
    Object.defineProperty(window, 'scrollY', { configurable: true, value: 64 });
    window.dispatchEvent(new Event('scroll'));

    await waitFor(() => {
      expect(screen.getByRole('banner')).toHaveClass('is-over-hero', 'is-hero-scrolled');
    });
  });
  ...
  it('uses the immersive centered hero without workflow-detail panels', () => {
    ...
    expect(screen.getByRole('region', { name: 'RadiantLogiq immersive hero' })).toBeVisible();
    expect(screen.queryByText('Clinical logic workspace')).not.toBeInTheDocument();
